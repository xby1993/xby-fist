...
import ui.NoteFrame;
import util.TransferableFactory;
...
public class Edit {
	...
	// 实现剪切方法，包括右键菜单中的剪切方法
	public void Cut() {
		if(frame.getJJTextPane().getCaret().getDot()==frame.getJJTextPane().getCaret().getMark()){
			return;
		}
		String str = frame.getJJTextPane().getSelectedText();
	...
	public void Copy() {
		...
		String str = frame.getJJTextPane().getSelectedText();
		contents = (StringSelection) TransferableFactory.getTrans(str);
		clipboard.setContents(contents, null);
	}
	...
	public void Paste() {
		if (clipboard.isDataFlavorAvailable(DataFlavor.stringFlavor)) {
			try {
				...
				} else// 如果没有选中文字，则插入文字
				{
					frame.getJJTextPane()
							.getDocument()
							.insertString(
									frame.getJJTextPane().getCaretPosition(),
									content, null);
			...
			try {
				...
				int insert = frame.getJJTextPane().getCaretPosition();
//				 frame.getJJTextPane().insertIcon(new ImageIcon(img));
				Style imageStyle = new StyleContext().addStyle("Image", null);
				StyleConstants.setIcon(imageStyle, new ImageIcon(img));
				frame.getJJTextPane().getDocument()
						.insertString(insert, "Image", imageStyle);
		...
		if (str != null)// str==null是点击了取消按钮触发的事件
		{
			...
			if (str.trim().length() != 0 && pattern.matcher(str).matches()) {
				...
				if (row >= 1 && row <= text.length) {
					...
					JOptionPane.showMessageDialog(frame, "列数超出范围！", "错误提示",
							JOptionPane.ERROR_MESSAGE);
				}
			} else {
				JOptionPane.showMessageDialog(frame, "无效的字符！", "错误提示",
						JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	public void insetDatetime() {
		...
		try {
			frame.getMyDocument().insertString(pos,
					calendar.getTime().toString(), null);
		} catch (BadLocationException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
